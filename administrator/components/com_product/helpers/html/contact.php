...
abstract class JHtmlProduct
{
	...
	public static function association($productid)
	{
		...
		if ($associations = JLanguageAssociations::getAssociations('com_product', '#__product_details', 'com_product.item', $productid))
		{
			...
			// Get the associated product items
			$db = JFactory::getDbo();
			$query = $db->getQuery(true)
				->select('c.id, c.name as title')
				->select('l.sef as lang_sef')
				->from('#__product_details as c')
	 ...
	 *
	 * @return  string	The anchor tag to toggle featured/unfeatured productS.
	 *
	 * @since   1.6
	 */
	public static function featured($value = 0, $i, $canChange = true)
	{
		...
		$states = array(
			0 => array('unfeatured', 'productS.featured', 'COM_PRODUCT_UNFEATURED', 'JGLOBAL_TOGGLE_FEATURED'),
			1 => array('featured', 'productS.unfeatured', 'JFEATURED', 'JGLOBAL_TOGGLE_FEATURED'),
		);
