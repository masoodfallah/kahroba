...
class ProductModelProduct extends JModelAdmin
{
	 ...
	 * @since  3.4.4
	 */
	protected $associationsContext = 'com_product.item';
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return JFactory::getUser()->authorise('core.delete', 'com_product.category.' . (int) $record->catid);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		{
			return JFactory::getUser()->authorise('core.edit.state', 'com_product.category.' . (int) $record->catid);
		}
	...
	public function getItem($pk = null)
	{
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_product', '#__product_details', 'com_product.item', $item->id);
	...
	protected function loadFormData()
	{
		...
		if (empty($data))
		{
			...
			{
				$data->set('catid', $app->input->get('catid', $app->getUserState('com_product.productS.filter.category_id'), 'int'));
			}
		}
	...
	public function save($data)
	{
		...
		if ($catid > 0)
		{
			$catid = CategoriesHelper::validateCategoryId($data['catid'], 'com_product');
		}
	...
	public function featured($pks, $value = 0)
	{
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update('#__product_details');
	...
	{
		return JFactory::getUser()->authorise('core.create', 'com_product');
	}
}
